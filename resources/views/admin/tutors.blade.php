...
<div class="container" style="max-width:1100px;">
    ...
    <div class="mb-4 d-flex justify-content-between align-items-center flex-wrap gap-3">
        <div>
            ...
            <h1 style="color:#0f172a;font-size:2rem;font-weight:800;margin-bottom:0.2rem;">All Tutors</h1>
            <p style="color:#64748b;margin:0;">{{ $tutors->count() }} tutors registered on the platform.</p>
        </div>
            ...
            <table style="width:100%;border-collapse:collapse;font-size:0.85rem;">
                <thead>
                    <tr style="background:#f8fafc;border-bottom:2px solid #e2e8f0;">
                        ...
                        <th style="padding:1rem 1.25rem;text-align:left;color:#64748b;font-weight:700;font-size:0.72rem;text-transform:uppercase;letter-spacing:0.5px;">Verification</th>
                    ...
                    <tr style="border-bottom:1px solid #f1f5f9;transition:0.15s;" onmouseover="this.style.background='#f8fafc'" onmouseout="this.style.background='transparent'">
                        <td style="padding:1rem 1.25rem;">
                            <div class="d-flex align-items-center gap-3">
                                @php
                                    $pic = optional($profile)->profile_picture
                                ...
                                <div>
                                    ...
                                    <p style="color:#94a3b8;font-size:0.75rem;margin:0;">ID: {{ $tutor->id }}</p>
                                </div>
                            </div>
                        </td>
                        <td style="padding:1rem 1.25rem;color:#64748b;">{{ $tutor->email }}</td>
                        <td style="padding:1rem 1.25rem;color:#64748b;">{{ optional($profile)->contact_no ?: '—' }}</td>
                        <td style="padding:1rem 1.25rem;">
                            @php $status = optional($profile)->verification_status ?? 'PENDING'; @endphp
                            @if($status === 'APPROVED')
                                <span style="background:rgba(34,197,94,0.1);color:#16a34a;border-radius:999px;font-size:0.72rem;font-weight:700;padding:3px 12px;display:inline-flex;align-items:center;gap:4px;">
                                    <i class="bi bi-patch-check-fill"></i> Verified
                                </span>
                            @elseif($status === 'REJECTED')
                                <span style="background:rgba(239,68,68,0.1);color:#ef4444;border-radius:999px;font-size:0.72rem;font-weight:700;padding:3px 12px;">
                                    <i class="bi bi-x-circle me-1"></i>Rejected
                                </span>
                            @else
                                <span style="background:rgba(245,158,11,0.1);color:#d97706;border-radius:999px;font-size:0.72rem;font-weight:700;padding:3px 12px;">
                                    <i class="bi bi-hourglass-split me-1"></i>Pending
                                </span>
                            @endif
                        </td>
                        <td style="padding:1rem 1.25rem;color:#94a3b8;font-size:0.8rem;">{{ $tutor->created_at->format('d M Y') }}</td>
                    </tr>
                    @empty
                    <tr>
                        <td colspan="5" style="padding:3rem;text-align:center;color:#94a3b8;">No tutors registered yet.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
